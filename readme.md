...
| **睡眠** ⭐核心 | **三口径切换（全部 / 晚间 / 午睡）**：晚间 = 20:00–次日 06:00 开始的段，午睡 = 白天段；日/周/月三种粒度对比（◀ ▶ 切换）；单日时间轴 + 近 30 天堆叠趋势；周/月视图与上期并列对比；**睡眠质量评分表**（参考 Apple：时长 7.5h=100 理想、6h=60 及格，综合效率/深睡/规律加权）；**时长状态分级（<5h 严重不足 / 5–5.5h 不足 / >8h 偏高）**；**睡眠质量历史**（近 90 天评分折线 + 65 分参考线 + 改进建议）；阶段构成、入睡/醒来时间趋势、近 12 周周均趋势（含 5.5h/8h 参考线与目标线）、**睡眠 × HRV 双轴叠加图**（睡眠柱 + 睡眠期 HRV 折线）、睡眠周规律热力（多块纵向堆叠 + 分级配色）、腕温/目标附加数据 |
| **心率** ⭐核心 | 日/周/月粒度对比；平均/静息/最高/最低/HRV/峰值时段指标卡；区间带趋势图；心率分布直方图；强度分区占比（<60 / 60–100 / 100–140 / 140–180 / >180）；HRV 趋势、静息 vs 步行心率对比 |
| **其他指标** | **通用指标分析模块**：27+ 指标可选（核心：步数/能量/血氧/HRV/静息心率/呼吸/体重/爬楼/锻炼/站立/骑行/步态等；其他：步态不对称/体力活动/声学暴露等）；日/周/月粒度切换；日均/最高/最低/覆盖天数指标卡；趋势图、周规律热力、近 12 周统计摘要；底部指标清单可点击切换 |
...
实测（真实数据）：晚间口径 136 天有效效率（最高 100%）、6 天数据异常、234 天无在床记录（Apple 未记录 InBed 段）；2026-06-18 曾误显 1097% 已修正为「数据异常」/「—」。
